...
// Incluimos nuestra cabecera, que CONTIENE los pines y prototipos
...
/**
 * @brief Almacena el estado y las ganancias de un controlador PID.
 */
typedef struct {
    float kp, ki, kd;
...
/**
 * @brief Almacena toda la información de estado y hardware de un motor.
 */
typedef struct {
    motor_id_t id;
...
/**
 * @brief Array estático que contiene los objetos de los 3 motores.
 * 'static' lo hace privado a este archivo (motor_control.c).
 */
...
/**
 * @brief Inicializa los parámetros de un controlador PID.
 */
...
/**
 * @brief Calcula la salida del PID basándose en el setpoint y el valor actual.
 * @param pid Puntero al struct del controlador PID.
 * @param setpoint El valor deseado (ángulo objetivo).
 * @param measured_value El valor actual (ángulo medido).
 * @return La señal de control (velocidad) calculada.
 */
static float pid_compute(pid_controller_t *pid, float setpoint, float measured_value) {
    ...
    if (dt <= 0.0f) return 0.0f; // Evitar división por cero
...
/**
 * @brief Reinicia el estado del PID (error previo e integral).
 */
...
/**
 * @brief ISR (Rutina de Interrupción) que maneja los encoders.
 * Se ejecuta en cada flanco del pin A para contar pulsos y determinar dirección.
 * @param arg El 'motor_id_t' (0, 1, o 2) del motor que generó la interrupción.
 */
...
/**
 * @brief Controla la velocidad y dirección del motor (bajo nivel).
 * @param motor_id El motor a controlar.
 * @param speed Valor de -255 a 255 (para 8 bits) que indica velocidad y dirección.
 */
...
/**
 * @brief Tarea de FreeRTOS que ejecuta el bucle de control PID.
 * Esta tarea se encarga de los 3 motores.
        ...
        for (int i = 0; i < NUM_MOTORS; i++) {
            if (motors[i].position_control_active) {
                ...
                if (fabs(error) < 1.0f) {
                    ...
                    set_motor_speed(i, 0);
                    continue; // Pasa al siguiente motor
                }
 ...
 * (Implementación de la función declarada en motor_control.h)
 */
void update_all_pid_parameters(float kp, float ki, float kd, int limit_percent)
{
    ...
    // Calcular el nuevo límite de salida (0-255 para 8 bits)
    float max_output = (limit_percent / 100.0f) * 255.0f;

    // Bucle para actualizar los parámetros en las estructuras internas de cada motor
    for (int i = 0; i < NUM_MOTORS; i++) {
        motors[i].pid.kp = kp;
        motors[i].pid.ki = ki;
        motors[i].pid.kd = kd;
        ...
        // Resetear el estado del PID para una transición suave
        pid_reset(&motors[i].pid);
    }
    
    printf("\n[PID CONTROL] Parámetros actualizados:\n");
    printf("           ├─ Ganancias: KP=%.2f, KI=%.2f, KD=%.2f\n", kp, ki, kd);
    printf("           └─ Límite de salida: %d%% (MAX: %.0f)\n", limit_percent, max_output);
}

/**
 * @brief Convierte los pulsos del encoder en ángulo en grados.
 * (Implementación de la función declarada en motor_control.h)
 */
float motor_get_current_angle(motor_id_t motor_id) {
    if (motor_id >= NUM_MOTORS) return 0.0f;
    // Factor 2 por la decodificación en cuadratura (ver ISR)
...
/**
 * @brief Mueve el motor a un ángulo objetivo (absoluto).
 * (Implementación de la función declarada en motor_control.h)
 */
void motor_move_relative(motor_id_t motor_id, float angle_change) {
    ...
    // printf("Motor %d - Nuevo objetivo: %.2f grados\n", motor_id, angle_change);
}

/**
 * @brief Detiene inmediatamente el control de posición del motor.
 * (Implementación de la función declarada en motor_control.h)
 */
...
/**
 * @brief Inicializa todo el sistema: motores, encoders, PWM y tarea de control.
 * (Implementación de la función declarada en motor_control.h)
 */
void motor_control_init(void) {
    ...
    // Inicializar estado y PID de cada motor (usando valores globales de main.c)
    for (int i = 0; i < NUM_MOTORS; i++) {
    ...
    for (int i = 0; i < NUM_MOTORS; i++) {
        ...
        // Configurar encoder pin A (Interrupción)
        ...
        // Configurar encoder pin B (Solo lectura)
